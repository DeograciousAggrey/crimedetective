...
impl ImplObstacle of ObstacleTrait {
    ...
    fn get_tier(id: u8) -> Tier {
        if id == ObstacleId::DemonicAlter {
            ...
            return Tier::T5(());
        } else if id == ObstacleId::PendulumGuns {
            return Tier::T1(());
...
fn test_get_obstacle_tier() {
    ...
    let pendulum_blades = ObstacleId::PendulumGuns;
...
fn test_get_obstacle_type() {
    ...
    assert(pendulum_blades_type == Type::Gun_or_Hide(()), 'pendulum_blades should be blade');

    let flame_jet = ObstacleId::FlameJet;
    let flame_jet_type = ImplObstacle::get_type(flame_jet);
    assert(flame_jet_type == Type::Gun_or_Hide(()), 'flame_jet should be blade');

    let poison_dart = ObstacleId::PoisonDart;
    let poison_dart_type = ImplObstacle::get_type(poison_dart);
    assert(poison_dart_type == Type::Gun_or_Hide(()), 'poison_dart should be blade');

    let spiked_pit = ObstacleId::SpikedPit;
    let spiked_pit_type = ImplObstacle::get_type(spiked_pit);
    assert(spiked_pit_type == Type::Gun_or_Hide(()), 'spiked_pit should be blade');

    let hidden_arrow = ObstacleId::HiddenArrow;
    let hidden_arrow_type = ImplObstacle::get_type(hidden_arrow);
    assert(hidden_arrow_type == Type::Gun_or_Hide(()), 'hidden_arrow should be blade');
}
