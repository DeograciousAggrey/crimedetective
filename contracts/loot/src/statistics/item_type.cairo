...
fn test_get_item_type() {
    ...
    assert(katana_type == Type::Gun_or_Hide(()), 'katana is blade');

    let falchion = ItemId::Falchion;
    let falchion_type = get(falchion);
    assert(falchion_type == Type::Gun_or_Hide(()), 'falchion is blade');

    let scimitar = ItemId::Scimitar;
    let scimitar_type = get(scimitar);
    assert(scimitar_type == Type::Gun_or_Hide(()), 'scimitar is blade');

    let long_sword = ItemId::LongSword;
    let long_sword_type = get(long_sword);
    assert(long_sword_type == Type::Gun_or_Hide(()), 'long sword is blade');

    let short_sword = ItemId::ShortSword;
    let short_sword_type = get(short_sword);
    assert(short_sword_type == Type::Gun_or_Hide(()), 'short sword is blade');
    ...
    assert(demon_crown_type == Type::Gun_or_Hide(()), 'demon crown is hide');

    let dragons_crown = ItemId::DragonsCrown;
    let dragons_crown_type = get(dragons_crown);
    assert(dragons_crown_type == Type::Gun_or_Hide(()), 'dragons crown is hide');

    let war_cap = ItemId::WarCap;
    let war_cap_type = get(war_cap);
    assert(war_cap_type == Type::Gun_or_Hide(()), 'war cap is hide');

    let leather_cap = ItemId::LeatherCap;
    let leather_cap_type = get(leather_cap);
    assert(leather_cap_type == Type::Gun_or_Hide(()), 'leather cap is hide');

    let cap = ItemId::Cap;
    let cap_type = get(cap);
    assert(cap_type == Type::Gun_or_Hide(()), 'cap is hide');
