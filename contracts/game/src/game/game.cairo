...
mod Game {
    ...
    use criminals::criminal::{Criminal, ICriminal, ImplCriminal};
    ...
    enum Event {
        ...
        DiscoverCriminal: DiscoverCriminal,
        AttackCriminal: AttackCriminal,
        SlayedCriminal: SlayedCriminal,
    ...
    impl Game of IGame<ContractState> {
        ...
        fn attack(ref self: ContractState, adventurer_id: u256) {
            ...
            // assert adventurer has a criminal to attack
        ...
        fn flee(ref self: ContractState, adventurer_id: u256) {
            ...
            // can't flee from start criminal
            _assert_not_starter_criminal(@self, adventurer);
    ...
    fn _explore(ref self: ContractState, ref adventurer: Adventurer, adventurer_id: u256) {
        ...
        match explore_result {
            ExploreResult::Criminal(()) => {
                ...
                // encounter criminal and check if adventurer was ambushed
                let (criminal, was_ambushed) = ImplCriminal::criminal_encounter(
                ...
                // initialize the criminal health. This is the only timeD criminal.starting_health should be 
                // used. In subsequent calls to attack the criminal, adventurer.criminal_health should be used as the persistent
                // storage of the criminal health
                adventurer.criminal_health = criminal.starting_health;
                ...
                if (was_ambushed) {
                    ...
                    let damage_taken = _criminal_counter_attack(
                        ref self,
                        ref adventurer,
                        adventurer_id,
                ...
                // Emit Discover Criminal event
                __event__DiscoverCriminal(
                    ref self,
                    DiscoverCriminal {
                        ...
                        id: criminal.id,
                        level: criminal.combat_spec.level,
                        ambushed: was_ambushed,
                        damage_taken: damage_taken,
                        health: criminal.starting_health,
                        prefix1: criminal.combat_spec.special_powers.prefix1,
                        prefix2: criminal.combat_spec.special_powers.prefix2,
                    }
                );
                ...
                if (adventurer.health == 0) {
                    ...
                    // note we technically could do this inside of _criminal_counter_attack but
                    // doing so would result in AdventurerDied being emitted before
                    // the DiscoverCriminal for the criminal that killed them. 
                    __event_AdventurerDied(
                        ...
                        killed_by_criminal: true,
                        killed_by_obstacle: false,
                        killer_id: criminal.id
                    );
                }
            },
    ...
    fn _criminal_discovery(ref self: ContractState, adventurer_id: u256) {}
    ...
    fn _attack(
        ...
        let criminal_prefix1 = U128TryIntoU8::try_into(
            battle_fixed_entropy % U8IntoU128::into(NamePrefixLength)
        )
            .unwrap();
        let criminal_prefix2 = U128TryIntoU8::try_into(
            battle_fixed_entropy % U8IntoU128::into(NameSuffixLength)
        )
            .unwrap();

        // use the randomly generated prefixes but set suffic to 0
        let criminal_name_prefix = SpecialPowers {
            prefix1: criminal_prefix1, prefix2: criminal_prefix2, suffix: 0
        };
        ...
        // get battle fixed criminal. The key to this is using battle fixed entropy
        let adventurer_level = adventurer.get_level();
        let criminal = ImplCriminal::get_criminal(adventurer_level, criminal_name_prefix, battle_fixed_entropy);
        if (adventurer.get_level() == 1) {
            let criminal = ImplCriminal::get_starter_criminal(weapon_combat_spec.item_type);
        }

        // get game entropy from storage
        let game_entropy: u64 = _get_entropy(@self).try_into().unwrap();

        // When generating the criminal, we need to ensure entropy remains fixed for the battle
        // for attacking however, we should change the entropy during battle so we use adventurer and criminal health
        // to accomplish this
        let attack_entropy = U64IntoU128::into(
            game_entropy
                + adventurer_entropy
                + U16IntoU64::into(adventurer.health + adventurer.criminal_health)
        );
        let damage_dealt = criminal
            .attack(weapon_combat_spec, adventurer.get_luck(), adventurer.strength, attack_entropy);
        // if the amount of damage dealt to criminal exceeds its health
        if (damage_dealt >= adventurer.criminal_health) {
            // the criminal is dead so set health to zero
            adventurer.criminal_health = 0;

            // grant equipped items and adventurer xp for the encounter
            let xp_earned = criminal.get_xp_reward();
            ...
            let gold_reward = criminal.get_gold_reward(battle_fixed_entropy);
            adventurer.increase_gold(gold_reward);

            // emit slayed criminal event
            __event__SlayedCriminal(
                ref self,
                SlayedCriminal {
                    adventurer_state: AdventurerState {
                        owner: get_caller_address(),
                    ...
                    prefix_1: criminal.combat_spec.special_powers.prefix1,
                    prefix_2: criminal.combat_spec.special_powers.prefix2,
                    criminal_level: criminal.combat_spec.level,
                    criminal_health: adventurer.criminal_health,
            ...
            // criminal has more health than was dealt so subtract damage dealt
            adventurer.criminal_health = adventurer.criminal_health - damage_dealt;

            // then handle the criminal counter attack
            ...
            let damage_taken = _criminal_counter_attack(
                ref self, ref adventurer, adventurer_id, attack_location, criminal, attack_entropy
            );

            // if adventurer health is zero (criminal_counter_attack checks for underflow)
            if (adventurer.health == 0) {
                // emit adventurer died
                // note we technically could do this inside of _criminal_counter_attack but
                // doing so would result in AdventurerDied being emitted before
            ...
            // emit attack criminal event
            __event__AttackCriminal(
                ref self,
                AttackCriminal {
                    adventurer_state: AdventurerState {
                        owner: get_caller_address(),
    ...
    fn _criminal_counter_attack(
        ...
        // generate a random attack slot for the criminal and get the armor the adventurer has at that slot
        ...
        // process criminal counter attack
        let damage_taken = criminal.counter_attack(armor_combat_spec, entropy);
    ...
    fn _flee(
        ...
        let fled = ImplCriminal::attempt_flee(
            adventurer.get_level(), adventurer.dexterity, flee_entropy
        );

        // our fixed battle entropy which we use to generate same criminal during a single battle
        let battle_fixed_entropy: u128 = adventurer.get_battle_fixed_entropy(adventurer_entropy);
        // here we save some compute by not looking up the criminal's special names during a failed flee
        // since they won't impact damage
        let criminal_name_prefix = SpecialPowers { prefix1: 0, prefix2: 0, suffix: 0 };
        let criminal = ImplCriminal::get_criminal(
            adventurer.get_level(), criminal_name_prefix, battle_fixed_entropy
        );
        let mut damage_taken = 0;
        let mut attack_location = 0;
        if (fled) {
            // set criminal health to zero to denote adventurer is no longer in battle
            adventurer.criminal_health = 0;
        } else {
            // if flee attempt was unsuccessful
            // the criminal will counter attack
            ...
            damage_taken =
                _criminal_counter_attack(
                    ref self, ref adventurer, adventurer_id, attack_slot, criminal, flee_entropy
                );
        }
    ...
    fn _assert_in_battle(self: @ContractState, adventurer: Adventurer) {
        assert(adventurer.criminal_health > 0, messages::ATTACK_CALLED_OUTSIDE_BATTLE);
    }
    fn _assert_not_starter_criminal(self: @ContractState, adventurer: Adventurer) {
        assert(adventurer.get_level() > 1, messages::CANT_FLEE_STARTER_CRIMINAL);
    }
    ...
    fn __event__DiscoverCriminal(ref self: ContractState, discover_criminal_event: DiscoverCriminal, ) {
        self.emit(Event::DiscoverCriminal(discover_criminal_event));
    }

    fn __event__AttackCriminal(ref self: ContractState, attack_criminal: AttackCriminal, ) {
        self.emit(Event::AttackCriminal(attack_criminal));
    }

    fn __event__SlayedCriminal(ref self: ContractState, slayed_criminal: SlayedCriminal, ) {
        self.emit(Event::SlayedCriminal(slayed_criminal));
    }
        ...
        self
            .emit(
                Event::AdventurerDied(
                    AdventurerDied {
                        adventurer_state, killed_by_criminal, killed_by_obstacle, killer_id
                    }
                )
            );
    }
