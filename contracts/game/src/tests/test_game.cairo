...
mod tests {
    ...
    fn test_start() {
        ...
        assert(adventurer_1.criminal_health > 0, 'criminal_health');
    ...
    fn test_attack() {
        ...
        assert(
            adventurer_start.criminal_health == CriminalSettings::STARTER_CRIMINAL_HEALTH,
            'wrong criminal starting health'
        );

        // attack criminal
        game.attack(0);

        // verify criminal and adventurer took damage
        let updated_adventurer = game.get_adventurer(0);
        assert(
            updated_adventurer.criminal_health < adventurer_start.criminal_health,
            'criminal should have taken dmg'
        );

        // if the criminal was killed in one hit
        if (updated_adventurer.criminal_health == 0) {
            ...
            // attack again (will take out starter criminal with current settings regardless of critical hit)
            game.attack(0);

            // recheck adventurer stats
            let updated_adventurer = game.get_adventurer(0);
            assert(updated_adventurer.criminal_health == 0, 'criminal should be dead');
            assert(updated_adventurer.xp > adventurer_start.xp, 'should have same xp');
            assert(updated_adventurer.gold > adventurer_start.gold, 'should have same gold');
    ...
    fn test_flee_starter_criminal() {
        let mut game = new_adventurer();
        let adventurer_start = game.get_adventurer(0);

        // attempt to flee starter criminal - should_panic
        game.flee(0);
    }
    ...
    fn test_flee() {
        ...
        // double tap the first criminal
        // TODO: Need to determine why starter criminal
        ...
        // send the 5x attack so I can discover a non-starter criminal
        game.attack(0);
        game.attack(0);
        game.attack(0);
        game.attack(0);
        ...
        // manipulate game entrop so we discover another criminal
        game.set_entropy(2);
        game.explore(0);
        let updated_adventurer = game.get_adventurer(0);
        assert(updated_adventurer.criminal_health != 0, 'should have found a criminal');

        game.set_entropy(4);
        game.flee(0);
        let updated_adventurer = game.get_adventurer(0);
        assert(updated_adventurer.criminal_health == 0, 'should have fled criminal');
    }
